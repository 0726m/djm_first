	...
	layui.use(['form', 'table','laydate','upload'], function () {
      ...
      //监听提交事件，其中data.filed就是需要提交的表单数据
        form.on('submit(add)', function (data) {
            $.post("<%=Const.ROOT%>kind/add",data.field,function(data){
            	if(data.code=="0"){
              ...
              //表单初始赋值
               ...
               setFormValue(obj,data);    
                ...
                layer.confirm('真的删除行么', function (index) {
                    ...
                    $.getJSON('<%=Const.ROOT%>kind/delete',{'id':obj.data.id},function(msg) {
  						//console.log(msg);
  						if(msg.code=="0"){
              ...
              form.on('submit(update)', function(message) {
                  ...
                  url:'<%=Const.ROOT%>kind/update',
					...
					success : function(msg) {
						...
						if(msg.code=="0"){
							layer.msg(msg.msg, { icon: 6, time: 800},function(){
								//location.href="<%=Const.ROOT%>kind.jsp";
								table.reload('currentTableId');//数据表格重
								layer.closeAll();//关闭弹出层
							});
						}else{
